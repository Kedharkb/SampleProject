		...
		return {
			...
			link: function (scope, elem, attrs, controller) {
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					selectTemplate = '<select ng-options="' + attrs.ngOptions + '" ng-model="' + attrs.ngModel + '" ' + (attrs.ngChange ? 'ng-change="' + attrs.ngChange + '"' : '') + '></select>',
					dropdownTemplate =
					...
					'<div class="dropdown-menu">' +
						'<div stop-propagation="click" class="custom-select-search">' +
							'<input class="' + attrs.selectClass + ' form-control" type="text" autocomplete="off" ng-model="' + searchModel + '" />' +
						'</div>' +
						'<ul role="menu">' +
							...
							'<li ng-hide="(' + values + (remoteSearch ? '' : ' | filter: search') + ').length" class="empty-result" stop-propagation="click">' +
								'<em class="muted">' +
									...
									'<span class="word-break" ng-show="' + searchModel + '">{{emptySearchResultText | format:' + searchModel + '}} <i class="fa fa-spinner fa-pulse pull-right"></i></span>' +
								'</em>' +
							'</li>' +
						'</ul>' +
