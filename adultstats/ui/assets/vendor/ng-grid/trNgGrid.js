...
(function (TrNgGrid) {
    ...
    var GridController = (function () {
        ...
        GridController.prototype.setupGrid = function (gridScope, gridOptions, isInServerSideMode) {
            ...
            if (gridOptions.enablePagination === undefined) {
    ...
    function configureTemplates($templateCache) {
        ...
        if (!$templateCache.get(TrNgGrid.cellHeaderTemplateId)) {
            $templateCache.put(TrNgGrid.cellHeaderTemplateId,
                ...
                '<div ' + TrNgGrid.columnSortDirectiveAttribute + '=""></div>' + '    </div>' +
                '<div ' + TrNgGrid.columnFilterDirectiveAttribute + '=""></div>' + '  </div>' + '</div>');
        }
        if (!$templateCache.get(TrNgGrid.cellBodyTemplateId)) {
            $templateCache.put(TrNgGrid.cellBodyTemplateId,
                ' <div ng-attr-class="' + TrNgGrid.bodyCellCssClass +
                ' text-{{columnOptions.displayAlign}} " ng-switch="isCustomized">' +
                ' <div ng-switch-when="true"> ' +
                ' <div ng-transclude=""></div> ' +
                '</div>' +
        ...
        if (!$templateCache.get(TrNgGrid.columnFilterTemplateId)) {
            $templateCache.put(TrNgGrid.columnFilterTemplateId,
                ...
                '<input class="form-control input-sm text-{{columnOptions.displayAlign}}" type="text" ng-model="columnOptions.filter" ng-keypress="speedUpAsyncDataRetrieval($event)" placeholder="{{columnOptions.columnPlaceholder}}"></input>' +
                '</div>' + '</div>');
        }
